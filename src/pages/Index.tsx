...
const Index = () => {
  const {
    ref: tournamentsRef,
    isVisible: isTournamentsVisible
  } = useAnimation();
  const {
    ref: matchesRef,
    isVisible: isMatchesVisible
  } = useAnimation();
  return <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow">
        ...
        {/* Upcoming Matches */}
        <section ref={matchesRef as React.RefObject<HTMLElement>} className="py-20 bg-white">
          <div className="container max-w-7xl mx-auto px-4 sm:px-6">
            <div className={cn("transition-all duration-700 transform", isMatchesVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-20")}>
              ...
              <div className="space-y-4">
                {upcomingMatches.map(match => <div key={match.id} className="flex flex-col md:flex-row justify-between items-center p-6 bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
